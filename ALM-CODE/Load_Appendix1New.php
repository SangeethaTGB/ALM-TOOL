...
$connect_112 = mysqli_connect("10.88.1.112","root","","balancesheet_2022_23_may");
...
<?php
if (isset($_POST['submitTRBNew']))
{
$asondate=$_POST['asondate'];
$dw_table="investments.dailyweekly_31052022";

//$asondate=date("Y-m-t",strtotime($asondate));
$sql1="select * from balancesheet_2022_23_may.cgl_ar1_mapping";
$res1=mysqli_query($connect_112,$sql1);
...
//Outflows - Capital  - 1
$sql1="UPDATE investments.alm_appendix1_maturity_profile_liquidity a,  
(SELECT sum(abs(d.balance)) bal FROM investments.cgl_ar1_mapping c, $dw_table d where d.cgl_acc_no=c.cgl and c.ar1_mapping in ('SHARE_CAPITAL','SHARE_CAPITAL_DEPOSITS') ) b
SET RM_OVER_5Y=b.bal, RM_TOTAL=b.bal WHERE DisplayOrder=1 AND asondate='$asondate' ";
echo $sql1;
$res1=mysqli_query($conn,$sql1);
...
//Outflows - RESERVES AND SURPLUS - 2
$sql1="UPDATE investments.alm_appendix1_maturity_profile_liquidity a,
(SELECT sum(abs(d.balance)) bal FROM investments.cgl_ar1_mapping c, $dw_table d where d.cgl_acc_no=c.cgl and c.ar1_mapping in ('STATUTORY_RESERVES','CAPITAL_RESERVES','REVENUE_OTHER_RESERVES','PROFIT_LOSS','PROFIT_LOSS_CURRENT_YEAR') )  b 
SET RM_OVER_5Y=b.bal, RM_TOTAL=b.bal WHERE DisplayOrder=2 AND asondate='$asondate' ";
echo $sql1;
$res1=mysqli_query($conn,$sql1);
...
//Outflows - Current Deposits - 3.1
$sql1="UPDATE investments.alm_appendix1_maturity_profile_liquidity a, 
( SELECT sum(abs(d.balance)) bal FROM investments.cgl_ar1_mapping c, $dw_table d where d.cgl_acc_no=c.cgl and c.ar1_mapping in ('CURRENT_ACCOUNT') ) b 
SET RM_1D_to_14D=b.bal*0.15,RM_1Y_to_3Y=b.bal*0.85, RM_TOTAL=b.bal WHERE DisplayOrder=4 AND asondate='$asondate' ";
$res1=mysqli_query($conn,$sql1);

//Outflows - Savings Bank - 3.2
$sql1="UPDATE investments.alm_appendix1_maturity_profile_liquidity a,
(SELECT sum(abs(d.balance)) bal FROM investments.cgl_ar1_mapping c, $dw_table d where d.cgl_acc_no=c.cgl and c.ar1_mapping in ('SAVINGS_BANK','CREDIT_BAL_CCOD','INT_ON_RD_TDR_STDR') ) b  
 SET RM_1D_to_14D=b.bal*0.10,RM_1Y_to_3Y=b.bal*0.90, RM_TOTAL=b.bal WHERE DisplayOrder=5 AND asondate='$asondate' ";
$res1=mysqli_query($conn,$sql1);
...
//Outflows - Refinances - 4.3
$sql1="UPDATE investments.alm_appendix1_maturity_profile_liquidity a,
(SELECT sum(abs(d.balance)) bal FROM investments.cgl_ar1_mapping c, $dw_table d where d.cgl_acc_no=c.cgl and c.ar1_mapping in ('SBI_REFINANCE','NABARD_REFINANCE','MUDRA_REFINANCE','NHB_REFINANCE','NSFDC_REFINANCE','NSKFDC_REFINANCE','NBCFDC_REFINANCE','NSTFDC_REFINANCE') ) b 
 SET RM_TOTAL=b.bal WHERE DisplayOrder=11 AND asondate='$asondate' ";
$res1=mysqli_query($conn,$sql1);
...
//Outflows - Refinances - 4.4
$sql1="UPDATE investments.alm_appendix1_maturity_profile_liquidity a,
(SELECT sum(abs(d.balance)) bal FROM investments.cgl_ar1_mapping c, $dw_table d where d.cgl_acc_no=c.cgl and c.ar1_mapping in ('BORROWING_OTHER_BANKS') ) b 
 SET RM_TOTAL=b.bal WHERE DisplayOrder=12 AND asondate='$asondate' ";
$res1=mysqli_query($conn,$sql1);
...
//Outflows - Bills Payables - 5.1
$sql1="UPDATE investments.alm_appendix1_maturity_profile_liquidity a,
(SELECT sum(abs(d.balance)) bal FROM investments.cgl_ar1_mapping c, $dw_table d where d.cgl_acc_no=c.cgl and c.ar1_mapping in ('DRAFTS_BTW_CORE','BANKERS_CHEQUE') ) b 
-- (SELECT SUM(ABS(balance)) bal FROM mis.dailyweekly WHERE level3 IN ('BANKERS CHEQUE AMT TRF BY BRAN','BANKERS CHEQUES','DRAFT AC-BETWEEN CORE BRANCHES') ) b 
 SET RM_1D_to_14D=b.bal, RM_TOTAL=b.bal WHERE DisplayOrder=14 AND asondate='$asondate' ";
$res1=mysqli_query($conn,$sql1);
...
$row1=mysqli_fetch_row($res1);
// $tot1=$row1[0]+$row1[1]+$row1[2]+$row1[3]+$row1[4]+$row1[5]+$row1[6]+$row1[7];
$sql1="update investments.alm_appendix1_maturity_profile_liquidity set RM_1D_to_14D='$row1[0]',RM_OVER_5Y='$row1[0]' where DisplayOrder=46 and asondate='$asondate' ";
$res1=mysqli_query($conn,$sql1);
...
{
$asondate=$_POST['asondate'];
$tblName = 'investments.npa_30062022';
$sucessCnt=0;
...
$sql="SELECT 
       ...
       ROUND(SUM(CASE WHEN (DATEDIFF(run_date,SAN_DT)>1825) THEN unavailed ELSE 0 END),2)  
		...
		UNION ALL 
		SELECT run_date,SAN_DT,BALANCE,sanclimit,IF(crdr='DR',REPLACE(`SancLimit`,',','')-BALANCE,REPLACE(`SancLimit`,',','')+BALANCE) AS 'unavailed'
		FROM $tblName n INNER JOIN  others.`prod_desc` ON (prod = account_type)  
...
Sum(A8) A8 FROM (select 
	  ...
	  from $tblName  
    where irac_o < 4 AND CRDR LIKE '%DR%'
 ...
 UNION 
  ...
  select 
       ...
       round(sum(case when (datediff(run_date,SAN_DT)>1095 and datediff(run_date,SAN_DT)<=1825) then BALANCE else 0 end),2) as A7,
       round(sum(case when (datediff(run_date,SAN_DT)>1825) then BALANCE else 0 end),2) as A8 
	 from $tblName  INNER JOIN  others.`prod_desc` ON 
   (prod = account_type )
  
  where irac_o < 4    AND CRDR LIKE '%DR%' AND 
  `SegmentName` = 'Demand Loans (DL)' and cat1 <20 )t  
	  ";
...
//Outflows - Interest Payable - 11 
	   $qry="SELECT SUM(c.intr_pbl) 
				FROM (SELECT t.Refinance_agency,t.Scheme,t.Interest,t.RefID,t.outStanding,MAX(r.inst_date),'2022-06-30',
				DATEDIFF(MAX(r.inst_date),'2022-06-30') AS 'days',((t.Interest/36500)*DATEDIFF(MAX(r.inst_date),'2022-06-30')*t.outStanding) intr_pbl
				FROM 
				(SELECT a.Refinance_agency,a.Scheme,a.Interest,a.RefID,a.Inst_amt+a.outStanding AS outStanding FROM refinance.inst_table a, 
				(SELECT MIN( Inst_date ) mindate,`RefID` rid FROM refinance.inst_table  WHERE Inst_date >= '2022-06-30' GROUP BY RefID)
...
?>
	...
	?></td>
			...
			<th>
				<!-- <input id="submitTRBId" name="submitTRB" class='btn btn-primary btn-lg' type="submit" value="Update TRB"  style="width:100%; display: <?php echo $disp; ?>"  /> -->
				<input id="submitTRBId" name="submitTRBNew" class='btn btn-primary btn-lg' type="submit" value="Update TRB New"  style="width:100%; display: <?php echo $disp; ?>"  />
			</th>
		</tr>
		<tr style="color: seagreen;">
			<th><?php echo $cnoCnt++; ?></th>
...
--><?php
echo "<center><br><table class='table table-hover table-bordered' border='1' style='width: 80%'>";      
$test=$test . "<center><br><table class='table table-hover table-bordered' border='1' >";      
echo "<tr>";
$test=$test . "<tr>";

$RM_1D_to_14D="1-14 Days <hr> " . date("d/m/Y") . " to " . date("d/m/Y",strtotime("+14 days")); 
$RM_15D_to_28D="15-28 Days <hr> " . date("d/m/Y",strtotime("+15 days")) . " to " . date("d/m/Y",strtotime("+28 days")); 
$RM_29D_to_3M="1-3 Months <hr> " . date("d/m/Y",strtotime("+29 days")) . " to " . date("d/m/Y",strtotime("+90 days")); 
$RM_3M_to_6M="3-6 Months <hr> " . date("d/m/Y",strtotime("+91 days")) . " to " . date("d/m/Y",strtotime("+180 days")); 
$RM_6M_to_1Y="6-12 Months <hr> " . date("d/m/Y",strtotime("+181 days")) . " to " . date("d/m/Y",strtotime("+365 days")); 
$RM_1Y_to_3Y="1-3 Years <hr> " . date("d/m/Y",strtotime("+366 days")) . " to " . date("d/m/Y",strtotime("+1095 days")); 
$RM_3Y_to_5Y="3-5 Years <hr> " . date("d/m/Y",strtotime("+1096 days")) . " to " . date("d/m/Y",strtotime("+1825 days")); 
$RM_OVER_5Y="Above 5 Years <hr> Beyond " . date("d/m/Y",strtotime("+1825 days")); 

echo "<th>S.No.</th><th>OrderNo</th><th>Segments</th>
		<th>$RM_1D_to_14D</th>
		<th>$RM_15D_to_28D</th>
		<th>$RM_29D_to_3M</th>
		<th>$RM_3M_to_6M</th>
